...
function toWorkTimeEntries(intervals: TimeInterval[]): WorkTimeEntry[] {
  return intervals
    .filter((itv) => itv.startTime && itv.endTime)
    .map((itv) => ({
      gioBatDau: itv.startTime,
      gioKetThuc: itv.endTime,
      soGio: computeIntervalHours(itv),
    }));
}
...
export const OptimizedTimeInput: React.FC<OptimizedTimeInputProps> = ({ onTimeChange }) => {
  ...
  useEffect(() => {
    ...
    if (sum > 0) {
      ...
      onTimeChange(sumStr, detectedShift, toWorkTimeEntries(intervals));
    } else {
      setTotalHours('');
      setShift('');
      onTimeChange('', '', []);
    ...
    <Card className="bg-blue-50 border-blue-200">
      <CardHeader className="pb-3">
        <div className="flex justify-between items-center">
          ...
          >
            <Plus size={16} />
            Thêm thời gian ({intervals.length}/10)
          </Button>
        </div>
      </CardHeader>
          ...
          return (
            <div key={index} className="p-4 border rounded-lg bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <Label>Số giờ (giờ, số thập phân)</Label>
