// MachineManagement.tsx - Quản lý Máy móc (ĐÃ SỬA DÙNG SUPABASE + PHÂN QUYỀN)
      ...
      try {
        ...
        for (let idx = 0; idx < json.length; idx++) {
          ...
          try {
            ...
            const newMaMay = `MAY${String(machines.length + addedCount + 1).padStart(3, '0')}`;
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-3.5">
                ...
                <div className="flex gap-2 pt-1">
                  <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs h-9" disabled={isSubmitting || (!canAdd && !editingId)}>
              ...
              {canAdd && (
                <div className="border-t border-slate-100 mt-4 pt-3 space-y-2">
                  ...
                  <Button variant="outline" className="w-full h-10 border-dashed border-2 text-xs text-slate-600" onClick={() => fileInputRef.current?.click()} disabled={isImporting}>
          ...
          <Card className="rounded-xl border border-slate-100 shadow-sm bg-white overflow-hidden">
            ...
            <CardContent className="p-0 overflow-x-auto">
              <div className="min-w-[1050px] space-y-0">
                <div className="grid grid-cols-[35px_85px_170px_90px_90px_90px_90px_90px_90px_75px] gap-1 px-3 py-2.5 bg-slate-50/80 border-b text-xs font-bold text-slate-700 items-center text-center">
                  {canDelete && (
                    <div className="flex justify-center">
                      <Checkbox checked={filteredMachines.length > 0 && selectedIds.length === filteredMachines.length} onCheckedChange={toggleSelectAll} className="h-3.5 w-3.5" />
                    </div>
                  )}
                ...
                {filteredMachines.length === 0 ? (
                  ...
                  filteredMachines.map((machine) => (
                    <div key={machine.id} className="grid grid-cols-[35px_85px_170px_90px_90px_90px_90px_90px_90px_75px] gap-1 px-3 py-2 items-center border-b text-xs hover:bg-slate-50/40 transition-colors">
                      {canDelete && (
                        <div className="flex justify-center">
                          <Checkbox checked={selectedIds.includes(machine.id)} onCheckedChange={() => toggleSelect(machine.id)} className="h-3.5 w-3.5" />
                        </div>
                      )}
                      ...
                      <div className="flex justify-center gap-0.5">
                        <Button size="icon" variant="ghost" className="h-7 w-7 text-blue-600 hover:bg-blue-50" onClick={() => handleEdit(machine)}>
                          <Edit2 className="w-3.5 h-3.5" />
                        </Button>
                        {canDelete && (
                          <Button size="icon" variant="ghost" className="h-7 w-7 text-red-500 hover:bg-red-50" onClick={() => handleDelete(machine.id)}>
