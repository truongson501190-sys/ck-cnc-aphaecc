...
import { Camera, UserCircle, Mail, Phone, MapPin, CheckCircle2 } from 'lucide-react';
...
export default function Profile() {
  ...
  const emailValid = useMemo(() => (email ? EMAIL_REGEX.test(email) : true), [email]);
  const phoneValid = useMemo(() => (phone ? PHONE_REGEX.test(phone) : true), [phone]);
  const canSave = Boolean(fullName.trim() && emailValid && phoneValid);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <p className="text-slate-700 dark:text-slate-200">Bạn cần đăng nhập để truy cập hồ sơ cá nhân.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 dark:bg-slate-950">
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100">Hồ sơ cá nhân</h1>
            <p className="text-sm text-slate-600 dark:text-slate-400">Cập nhật thông tin liên hệ và ảnh đại diện của bạn.</p>
          </div>
        </div>
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="flex flex-col items-center gap-4 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">{user.position || 'Nhân viên'}</p>
                  </div>
                  <div className="space-y-2 text-left text-sm text-slate-600 dark:text-slate-400">
                    <p><span className="font-medium text-slate-700 dark:text-slate-200">MSNV:</span> {user.msnv}</p>
                    <p><span className="font-medium text-slate-700 dark:text-slate-200">Vai trò:</span> {user.role}</p>
                    <p><span className="font-medium text-slate-700 dark:text-slate-200">Đơn vị:</span> {user.department}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    {!emailValid && <p className="text-sm text-rose-500">Email không hợp lệ.</p>}
                  </div>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    {!phoneValid && <p className="text-sm text-rose-500">Số điện thoại chỉ chứa 8-15 chữ số.</p>}
                  </div>
                  <div className="space-y-2">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <span>Thông tin luôn được cập nhật khi lưu.</span>
                  </div>
