...
export function ExactLayoutWarehouseExport({ onSubmit }: ExactLayoutWarehouseExportProps) {
    ...
    try {
      ...
      if (savedProjects) {
        ...
        if (Array.isArray(parsedProjects)) {
          setProjects(parsedProjects.map((p: RawRecord) => ({
            id: String(p.id ?? ''),
            tenDuAn: String(p.tenDuAn ?? p.tenKhachHang ?? p.maDuAn ?? ''),
            maDuAn: p.maDuAn != null ? String(p.maDuAn) : undefined,
            tenKhachHang: p.tenKhachHang != null ? String(p.tenKhachHang) : undefined,
            ghiChu: p.ghiChu != null ? String(p.ghiChu) : undefined,
  ...
  const handleAddCategory = () => {
    ...
    setFormData(prev => ({ ...prev, chungLoai: categoryToAdd.id, donVi: categoryToAdd.donVi, donGia: String(categoryToAdd.gia ?? 0) }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const validation = validateWarehouseSelection(selectedUserDepartment, formData.khoXuat);
    if (!validation.valid) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border">
            ...
            <div>
              ...
              <Select value={formData.duAn} onValueChange={(value) => handleInputChange('duAn', value)}>
                ...
                <SelectContent>
                  {projects.map((p) => (
                    <SelectItem key={p.id} value={p.maDuAn ?? p.id}>
                      {p.maDuAn ?? p.id} - {p.tenKhachHang ?? p.tenDuAn}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                  ...
                  <SelectContent>
                    {selectedUserDepartment ? (
                      ...
                      <SelectItem value="placeholder" disabled>
                        Vui lòng chọn người xuất trước
                      </SelectItem>
                    )}
                  </SelectContent>
                </Select>
