...
export function ExactLayoutOilExport({ onSubmit }: ExactLayoutOilExportProps) {
  ...
  const handleItemChange = (id: string, field: keyof WarehouseTransactionItem, value: string | number | undefined) => {
    setItems(items.map(item => {
      if (item.id === id) {
        const updatedItem: WarehouseTransactionItem = {
          ...item,
          ...(
            field === 'quantity'
              ? { [field]: Number(value) }
              : { [field]: value }
          ) as Partial<WarehouseTransactionItem>
        } as WarehouseTransactionItem;
        if (field === 'itemId') {
          ...
          if (selectedCategory) {
            ...
            updatedItem.totalValue = updatedItem.quantity * (updatedItem.price ?? 0);
          }
        }
        if (field === 'itemName') {
          ...
          const selectedCategory = categories.find(cat =>
            cat.tenLoai === strValue ||
            cat.tenChungLoai === strValue ||
            cat.maLoai === strValue ||
            cat.id === strValue
          );
          if (selectedCategory) {
          ...
          updatedItem.itemId = strValue; // Set itemId to the manual name for consistency
        }
            ...
            <div className="border rounded-lg overflow-x-auto">
              <table className="min-w-full w-full text-sm">
                ...
                <tfoot className="bg-orange-50 font-bold border-t border-orange-100">
                  <tr><td colSpan={5} className="p-3 text-right text-orange-800">Tổng cộng xuất dầu:</td><td className="p-3 text-orange-700 text-lg">{items.reduce((sum, i) => sum + (i.totalValue ?? 0), 0).toLocaleString('vi-VN')} VND</td><td></td></tr>
                </tfoot>
              </table>
            </div>
